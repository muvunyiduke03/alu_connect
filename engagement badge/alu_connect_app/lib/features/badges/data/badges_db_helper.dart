...
/// BadgesDbHelper — SQLite persistence for Task 3 (Proof of Engagement Badges).
///
/// Tables:
...
class BadgesDbHelper {
  ...
  Future<Database> _initDb() async {
    ...
    return await openDatabase(
      path,
      // FIX: bumped to version 2 to add event_attendance table
      version: 2,
      onCreate: _onCreate,
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // Add event_attendance table for users upgrading from v1
      await db.execute('''
  ...
  Future<void> insertBadge(BadgeModel badge) async {
  ...
  Future<List<BadgeModel>> getBadgesForStudent(String studentId) async {
    final db = await database;
  ...
  Future<bool> badgeExists(String studentId, String eventId) async {
    ...
    final result = await db.query(
      ...
      limit: 1,   // FIX: add LIMIT 1 for efficiency
    );
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Attendance log queries (for organizer dashboard, if needed by teammates)
  // ──────────────────────────────────────────────────────────────────────────
